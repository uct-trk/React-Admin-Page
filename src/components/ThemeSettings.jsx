    ...
    <div className="bg-half-transparent w-screen fixed nav-item top-0 right-0">
      <div className="float-right bg-white h-screen dark:text-gray-200 dark:[#484B52] w-400">
        ...
        <div className="flex-col border-t-1 border-color p-4 ml-4">
            ...
            <div className="mt-4">
              <input checked={currentMode === 'Light'} onChange={setMode} type="radio" id="light" name="theme" value="Light" className="cursor-pointer"/>
              <label htmlFor="light" className="ml-2 text-md cursor-pointer">Light</label>
            </div>

            <div className="mt-4">
              <input checked={currentMode === 'Dark'} onChange={setMode} type="radio" id="dark" name="theme" value="Dark" className="cursor-pointer"/>
              ...
              {themeColors.map((item,index) => (
                <TooltipComponent key={index} content={item.name} position="TopCenter">
                  <div className="relative mt-2 cursor-pointer flex gap-5 items-center">
                    <button onClick={() => setColor(item.color)} style={{backgroundColor: item.color}} type="button" className="h-10 w-10 rounded-full cursor-pointer">
